...
const getRevalidatedPosts = unstable_cache(
  async () => ({
    posts: readPosts(),
...
export default async function IsrPage() {
  const { posts, renderedAt, color } = await getRevalidatedPosts();

  return (
    <main className="max-w-2xl mx-auto mt-8 px-4">
      <h1>revalidate (30s)</h1>
      <p className="text-gray-600 text-sm mt-1 mb-5">
        Datos que se ctualizan cada 30 segundos
      </p>
      ...
      <div className="flex items-center gap-4 p-3 border border-gray-200 bg-gray-50 mb-6">
        ...
        <div className="flex flex-col gap-1 text-xs text-gray-500">
          ...
          <span className="text-gray-400">cambia cada 30s</span>
        </div>
      </div>
